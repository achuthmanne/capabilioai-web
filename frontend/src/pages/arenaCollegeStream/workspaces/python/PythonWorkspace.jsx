/**
 * PythonWorkspace.jsx — Career Workspace refactor (2026-08-19).
 *
 * The panel-registry architecture's second real workspace implementation.
 * Registered under panel_type "python_runner" in ../registry.js. Built for
 * the ML/AI Engineer role first (Python/pandas/scikit-learn missions), but
 * generic to any role a python_runner mission is authored for — nothing
 * here is ML-specific.
 *
 * Receives exactly one prop — `workspace` — same top-level shape every
 * workspace receives (see workspaces/sql/SqlWorkspace.jsx's own header),
 * with a workspace-specific `state`/`actions`/`permissions` shape:
 *   { mission, submission: {result, submitting, error},
 *     state: {code, setCode}, actions: {onSubmit, onOpenMission, onBackToMissions},
 *     permissions: {canSubmit}, navigation: {missions}, timer: {deadline} }
 * No `preview` — Submit-only this pass. The SQL workstation's non-scoring
 * "Run" preview relies on POST /missions/:id/validate's SQL-specific
 * response shape (columns/rows/row-count warnings); building a Python
 * equivalent needs that route generalized first rather than faking a
 * preview panel with the wrong data shape. Owns zero state of its own —
 * everything above is owned by ArenaCollegeStream.jsx and passed down,
 * same discipline as SqlWorkspace.
 */
...
export default function PythonWorkspace({ workspace }) {
  ...
  return (
    <div>
      <style>{`@media (max-width: 900px) { .domain-workspace-grid-py { grid-template-columns: 1fr !important; } }`}</style>
      ...
      <div className="domain-workspace-grid-py" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ background: T.cream2, border: `1px solid ${T.border}`, borderRadius: 16, padding: 22, boxShadow: T.shadow }}>
          <Eyebrow color={T.ink3}>Python Editor</Eyebrow>
          <div style={{ marginBottom: 12 }}>
            <PythonEditor value={code} onChange={setCode} disabled={submission.submitting} T={T} MONO={MONO} />
          </div>
